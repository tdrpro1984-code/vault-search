...
> ⚠️ Vault Curate 目前提交 Obsidian 社群審核中。社群外掛程式裡若搜不到，請暫時走下方 [手動安裝](#手動安裝) 步驟。
...
1.2.0 起相似度抗模板：embedding 輸入會剝除 markdown 結構符號（表格框線、`█▃▅` 條等），frontmatter 的 `description`（若有）會加權進筆記排名向量——人物卡找到的是**這個人**的對話，而不是九張同模板的兄弟卡。升級後首次啟動會自動做一次性增量重索引（含符號的筆記）+ description 向量補嵌，有進度通知、不需手動 Rebuild。
